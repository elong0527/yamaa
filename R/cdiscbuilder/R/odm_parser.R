...
parse_odm_to_long_df <- function(xml_file, xml_mapping = NULL) {
  ...
  ns_values <- as.character(raw_ns)
  matched <- ns_values[ns_values %in% supported_uris]
  if (length(matched) == 0) {
    stop(
      sprintf(
        "Unsupported ODM namespace: expected one of %s, found %s",
        paste(supported_uris, collapse = ", "),
        if (length(ns_values) == 0) "none" else paste(ns_values, collapse = ", ")
      )
    )
  }
  odm_uri <- matched[1]
  ns <- c(odm = odm_uri)
  item_metadata <- list()
  item_defs <- xml_find_all(doc, ".//odm:ItemDef", ns)
  for (item_def in item_defs) {
    oid <- xml_attr(item_def, "OID")
    name <- xml_attr(item_def, "Name")
    question <- ""
    q_node <- xml_find_first(item_def, "./odm:Question/odm:TranslatedText", ns)
    if (!is.na(q_node)) {
  ...
  data_rows <- list()
  clinical_data_nodes <- xml_find_all(doc, ".//odm:ClinicalData", ns)
  for (cd in clinical_data_nodes) {
    study_oid <- xml_attr(cd, merged_mapping$study_oid)
    subject_data_nodes <- xml_find_all(cd, "./odm:SubjectData", ns)
    for (sd in subject_data_nodes) {
      ...
      study_event_nodes <- xml_find_all(sd, "./odm:StudyEventData", ns)
      for (sed in study_event_nodes) {
        ...
        form_nodes <- xml_find_all(sed, "./odm:FormData", ns)
        if (length(form_nodes) == 0) {
          form_nodes <- xml_find_all(sed, "./odm:ItemGroupData", ns)
        }
        for (form in form_nodes) {
          ...
          ig_nodes <- xml_find_all(form, "./odm:ItemGroupData", ns)
          for (ig in ig_nodes) {
