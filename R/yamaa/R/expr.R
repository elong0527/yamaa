...
eval_expression <- function(expr, ctx) {
  ...
  switch(kind,
    ...
    str_pad = eval_str_pad(payload, ctx),
    round_half_away_from_zero = eval_round_half(payload, ctx),
    flag = eval_flag(payload, ctx),
...
# qualifiers used by id nodes in a parsed expression/predicate AST.
# include_star also collects reduce-star relations (aggregate scope check).
ast_quals <- function(node, include_star = FALSE) {
  qs <- character(0)
  walk <- function(x) {
    if (!is.list(x)) return()
    if (identical(x$kind, "id") && !is.null(x$qualifier))
      qs <<- c(qs, x$qualifier)
    if (include_star && identical(x$kind, "reduce") && !is.null(x$star))
      qs <<- c(qs, x$star)
    for (e in x) walk(e)
  }
  walk(node)
...
# qualifiers in a predicate (REQ-0132 scope check)
pred_quals <- function(node) ast_quals(node)

# Split a lookup filter into its donor-only and driver-correlated parts
# (REQ-0132/0133). Returns list(fnode, corr_q); corr_q is empty for a
filter_scope <- function(ctx, ds, filter, what) {
  fnode <- if (!is.null(filter)) parse_predicate_text(filter) else NULL
  corr_q <- setdiff(pred_quals(fnode), ds)
  if (length(corr_q) == 0) return(list(fnode = fnode, corr_q = corr_q))
  if (length(corr_q) > 1)
    yamaa_error("unknown_field", paste0(what, ": multiple driver qualifiers"))
  drv <- unique(ctx$driver_ds)
  if (length(drv) != 1 || corr_q != drv)
    yamaa_error("unknown_field", paste0("bad qualifier ", corr_q, " for ", what))
  list(fnode = fnode, corr_q = corr_q)
}

# dual-scope record resolver for a driver-correlated lookup filter
# (REQ-0120/0132/0133): donor-qualified names read the matched donor
make_correlated_resolver <- function(ctx, q, recs, ddrv, drec) {
  ydf <- ctx$inputs[[q]]; cts <- attr(ydf, "coltypes")
  dyf <- ctx$inputs[[ddrv]]; dcts <- attr(dyf, "coltypes")
  n <- length(recs)
  r1 <- if (length(drec)) drec[1] else NA_integer_
  resolve <- function(name) {
    s <- split_qual(name)
    # REQ-1260: in a SELF intermediate filter, a bare (unqualified) field
    # names the completed donor record, never the current row
    if (is.null(s$q) || s$q == q) {
      if (!s$v %in% names(ydf)) yamaa_error("unknown_field", paste0(name, ": no such field"))
      return(tv(ydf[[s$v]][recs], cts[[s$v]]))
    }
    if (s$q == ddrv) {
      if (!s$v %in% names(dyf)) yamaa_error("unknown_field", paste0(name, ": no such field"))
      if (length(drec) > 1 && !s$v %in% ctx$keys)
        yamaa_error("unknown_field", paste0(name, ": not a group key"))
      v <- if (is.na(r1)) tv_na(dcts[[s$v]], n) else tv(rep(dyf[[s$v]][r1], n), dcts[[s$v]])
      return(v)
    }
    yamaa_error("unknown_field", paste0("bad qualifier ", s$q, " for ", q, " records"))
  }
  list(resolve = resolve, n = n)
}
...
eval_first_available <- function(payload, ctx) {
  ...
  vals <- lapply(srcs, function(s)
    resolve_source_binding(normalize_source_binding(s), ctx))
...
# ---- case -----------------------------------------------------------------
# merge one case branch into the accumulator at the selected positions.
merge_case_branch <- function(out_v, out_t, branch, sel, n) {
  if (branch$t == "null") return(list(v = out_v, t = out_t))  # NA already
  if (out_t == "null") { out_t <- branch$t; out_v <- tv_na(out_t, n)$v }
  if (branch$t != out_t) {
    if (all(c(out_t, branch$t) %in% c("int", "float"))) {
      out_v <- convert_tv(tv(out_v, out_t), "float")$v; out_t <- "float"
    }
    out_v[sel] <- convert_tv(tv(branch$v[sel], branch$t), out_t)$v
  } else out_v[sel] <- branch$v[sel]
  list(v = out_v, t = out_t)
}
...
eval_case <- function(payload, ctx) {
  ...
  for (it in items) {
    ...
    if (any(take)) {
      branch <- eval_case_branch(it$then, ctx, take, n)
      if (is.null(out_t)) { out_t <- branch$t; out_v <- tv_na(out_t, n)$v }
      # null branches coerce to the established type
      mg <- merge_case_branch(out_v, out_t, branch, take, n)
      out_v <- mg$v; out_t <- mg$t
  ...
  if (is.null(out_t)) {
    ...
    branch <- eval_case_branch(otherwise, ctx, rep(TRUE, n), n)
    return(branch)
  }
  if (!is.null(otherwise)) {
    branch <- eval_case_branch(otherwise, ctx, !claimed, n)
    mg <- merge_case_branch(out_v, out_t, branch, !claimed, n)
    out_v <- mg$v; out_t <- mg$t
  }
  tv(out_v, out_t)
}

# eval_case_branch(then, ctx, take, n): a case branch is evaluated for the
# rows its condition selects. Eager evaluation runs first so an aggregate
# in the branch keeps its declared scope; if eager fails, the branch is
# retried on the taken rows alone, and a retry failure re-raises the
# original error. The retry result scatters back to full length.
eval_case_branch <- function(then, ctx, take, n) {
  branch <- tryCatch(eval_expression(then, ctx), error = function(e) e)
  if (!inherits(branch, "error")) return(branch)
  if (all(take)) stop(branch)
  sub <- ctx; sub$n <- sum(take)
  sub$col <- lapply(ctx$col, function(c)
    if (is.list(c) && !is.null(c$v) && length(c$v) == n) tv(c$v[take], c$t) else c)
  # per-row identity must follow the same rows: subset every row-indexed
  # field alongside col, or the retry resolves values from the wrong rows
  sub_row <- function(x) if (!is.null(x) && length(x) == n) x[take] else x
  sub$driver_ds <- sub_row(ctx$driver_ds)
  sub$driver_rec <- sub_row(ctx$driver_rec)
...
# resolve one normalized source binding to a tv: unqualified names read
# the current output column, intermediate qualifiers read the lookup, and
# dataset qualifiers read the scalar source (REQ-1057)
resolve_source_binding <- function(binding, ctx) {
  s <- split_qual(binding$variable)
...
eval_mapping_source <- function(source, ctx) {
  ...
  resolve_source_binding(normalize_source_binding(source), ctx)
}
...
eval_cut <- function(payload, ctx) {
  ...
  xv <- as.numeric(src$v)
  hit <- !is.na(xv)
  # REQ-0334: a missing cut input with no `missing:` handler is fatal
  if (any(!hit) && is.null(payload$missing))
    yamaa_error("missing_input", "cut: missing input without a missing: handler")
  if (any(hit)) {
...
collect_qualifiers <- function(node) ast_quals(node, include_star = TRUE)
...
eval_derive_bindings <- function(derive, ctx, ds, recs) {
  ...
  for (b in derive) {
    ...
    for (j in seq_len(nrec)) {
      ...
      # the one-record view drives from the relation itself, so a qualified
      # read of the relation sees this record, not the outer driver row
      ctx1$driver_ds <- ds; ctx1$driver_rec <- list(1L)
...
derive_relation <- function(payload, ctx) {
  ...
  if (!is.null(payload$filter)) {
    ...
    if (!is.null(fnode)) qs <- c(qs, pred_quals(fnode))
  }
...
# REQ-0588: absent `invalid` -> "ABSENT" (fail); explicit null -> NA
invalid_handler <- function(payload) {
  if (!"invalid" %in% names(payload)) return("ABSENT")
  ih <- payload$invalid
...
eval_date_impute <- function(payload, ctx) {
  ...
  # REQ-0592: month is required when minimum_source_precision is year
  # (the default) and must be absent when it is month.
  msp <- if (is.null(payload$minimum_source_precision)) "year" else payload$minimum_source_precision
  if (msp == "month" && !is.null(payload$month))
    yamaa_error("month_not_permitted", "month is unreachable under minimum_source_precision: month")
  if (msp == "year" && is.null(payload$month))
  ...
  op_date_impute(src, as.list(month), as.list(dayv), msp,
    nb,
    if (is.null(payload$missing)) NA_character_ else payload$missing,
    invalid_handler(payload),
    ctx)
}
...
# ---- str expressions ------------------------------------------------------
# one str transform: source must be a field reference naming str; `missing`
# replaces missing results (REQ-0308 family)
eval_str_transform <- function(kind, payload, ctx, op) {
  if (!is.character(payload$source))
    yamaa_error("invalid_field_type",
      paste0(kind, " source must be a field reference"))
  out <- op(resolve_name(payload$source, ctx))
  if (!is.null(payload$missing)) out$v[is.na(out$v)] <- payload$missing
  out
}
...
eval_str_pad <- function(payload, ctx) {
  if (!is.list(payload) || is.null(payload$source) || is.null(payload$width))
    yamaa_error("invalid_expression", "str_pad needs source and width")
  op_str_pad(resolve_name(payload$source, ctx), payload$width)
}

# flag: one-predicate flag derivation (REQ-1256/1257/1258). A bare string is
# the condition with default values; a mapping names condition (required),
# true_value (default "Y"), false_value, missing_value. TRUE takes
# true_value, FALSE takes false_value (missing when absent), UNKNOWN takes
# missing_value (missing when absent). missing_value is required when
# false_value is declared, else missing_value_required.
eval_flag <- function(payload, ctx) {
  n <- ctx$n
  if (is.character(payload) && length(payload) == 1)
    payload <- list(condition = payload)
  if (!is.list(payload) || is.null(payload$condition))
    yamaa_error("invalid_field_type", "flag needs a predicate string or a mapping with condition")
  cond_src <- payload$condition
  if (!is.character(cond_src) || length(cond_src) != 1)
    yamaa_error("invalid_predicate", "flag condition must be a predicate string")
  true_v <- if (is.null(payload$true_value)) "Y" else payload$true_value
  false_v <- payload$false_value
  missing_v <- payload$missing_value
  for (v in list(true_v, false_v, missing_v)) {
    if (!is.null(v) && (!is.character(v) || length(v) != 1))
      yamaa_error("invalid_field_type", "flag values must be strings")
  }
  if (!is.null(false_v) && is.null(missing_v))
    yamaa_error("missing_value_required",
      "flag with false_value needs missing_value for the UNKNOWN case")
  # REQ-1258: the condition parses as a predicate, or fails invalid_predicate
  node <- parse_predicate_text(cond_src)
  resolver <- list(resolve = function(name) resolve_name(name, ctx), n = n)
  cond <- eval_pred(node, resolver)
  out <- rep(NA_character_, n)
  out[!is.na(cond) & cond] <- true_v
  if (!is.null(false_v)) out[!is.na(cond) & !cond] <- false_v
  if (!is.null(missing_v)) out[is.na(cond)] <- missing_v
  tv(out, "str")
}
