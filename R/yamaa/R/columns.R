...
derive_columns <- function(spec, ctx) {
  ctx$phase <- "column"
  # fresh intermediate selection cache: row-phase selections were computed
  # for template blocks and must not be reused at full row count
...
deriv_refs <- function(deriv, colnames) {
  ...
  walk_string <- function(x, key = NULL, parent = NULL) {
      ...
      # REQ-1255/1256: a bare-string flag payload IS the predicate
      if (identical(key, "flag") && identical(parent, "flag")) {
        node <- tryCatch(parse_predicate_text(x), error = function(e) NULL)
        if (!is.null(node)) refs <<- c(refs, ast_col_refs(node, colnames))
        return()
      }
