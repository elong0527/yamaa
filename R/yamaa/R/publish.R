  ...
  if (!is.null(vrep) && nzchar(vrep))
    jobs[[length(jobs) + 1L]] <- list(target = file.path(dirname(out_path), vrep),
      write = function(p) write_typed_artifact(
        build_verification_report(spec, ctx), vreport_coltypes(), p))
  if (!failed && spec_has_warning(spec))
    jobs[[length(jobs) + 1L]] <- list(target = file.path(dirname(out_path),
        spec$output$violation_log),
      write = function(p) write_typed_artifact(
        build_violation_log(spec, ctx), vlog_coltypes(), p))
  if (!failed)
...
# caller renames it over the target. Returns the temp path. The temp file
...
build_violation_log <- function(spec, ctx) {
...
build_verification_report <- function(spec, ctx) {
