  ...
  switch(kind,
    ...
    previous_non_missing = {
      if (!is.character(payload$source))
        yamaa_error("invalid_field_type",
          "previous_non_missing source must be a field reference")
      src <- resolve_name(payload$source, ctx)
      out <- tv_na(src$t, n)
      for (p in parts) {
        o <- p$order; last <- NA
        # carry the type-appropriate NA
        last <- tv_na(src$t, 1)$v
        for (j in seq_along(o)) {
          out$v[o[j]] <- last
          v <- src$v[o[j]]
          if (!is.na(v)) last <- v
    ...
    baseline_flag = {
      ...
      for (p in parts) {
        ...
        out$v[o[elig]] <- ifelse(o[elig] == winners, "Y", "N")
      }
      out
    },
...
# list(rows = eligible row indices (input order), order = ordered indices,
#      tie = tie-group id per ordered position).
# Excluded rows (filter not TRUE) are absent from every partition and get
# missing from the caller.
