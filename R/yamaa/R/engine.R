...
run_spec <- function(spec_path, out_path, workflow_stack = character(0)) {
  ...
  ctx0 <- list(
    ...
    spec_dir = spec_dir,
    phase = "init",
    # REQ-1251: named window definitions from the root `windows` block
    named_windows = if (is.null(spec$windows)) list() else spec$windows,
...
validate_output_block <- function(spec, out_path) {
  ...
  check_profile(vlog, "output.warning_log")
  check_profile(vrep, "output.verification_log")
  ...
  if (spec_has_warning(spec) && (is.null(vlog) || !nzchar(vlog)))
    yamaa_error("missing_warning_log",
      "specification declares warning verifications but no output.warning_log (REQ-0391)")
...
validate_input_path <- function(p, spec_dir) {
  ...
  if (!file.exists(full) && !dir.exists(full)) {
    orig <- attr(p, "yamaa_orig")
    if (!is.null(orig)) {
      # REQ-0780 project-root retry: the layer's own spelling is retried
      # against the project root (the entry directory in this runner);
      # a file stored beside the writing layer is already covered above,
      # since the rebased form names that same location.
      ofull <- file.path(spec_dir, orig)
      if (file.exists(ofull) && !dir.exists(ofull))
        return(validate_input_path(orig, spec_dir))
    }
