...
#
# resolve_spec(entry_path) loads one spec file and, when it declares
# `parents:`, composes the inherited layers into a single resolved spec.
# Implements rules/specification/composition.md (REQ-0614..REQ-0661).
#
# Pipeline: normalize parent references -> depth-first traversal (cycle
# detection) -> per-layer version and entry-output checks -> canonicalize
# each layer (expand shorthands, drop parents) -> accumulate root and keyed
# member fields layer by layer -> rebase inherited relative paths to the
# entry file -> materialize verification defaults -> prune dead inputs,
# intermediates and columns -> order columns deterministically ->
# materialize in schema field order.
#
# A spec without `parents` resolves to itself: single-layer resolution is
# the identity, so existing behavior is unchanged.
...
# parents are visited depth-first left-to-right before the layer itself;
# a layer contributes once at its first visit; revisiting a layer whose
# contribution is complete skips it; revisiting a layer on the active path
...
# fails with schema_version_mismatch and reports every implicated file.
...
# check_entry_output(entry): the entry itself must declare complete
# non-null output; inherited output cannot satisfy this (REQ-0657).
check_entry_output <- function(entry) {
  if (is.null(entry$output))
    yamaa_error("missing_entry_output",
      "entry file omits output; inherited output cannot satisfy it")
}
...
# {value: {source: {variable: name}}}; a single expression written without
# the value wrapper ({case: ...}) becomes {value: <expression>}; a
...
# bare-string source payload to {variable: x}; expand case branches.
# Other expression kinds are kept whole: a bare string in a sequence is a
# filtered_source element (bare variable), not a source expression.
...
# paths keep their target; the rebased form is what Resource resolution
# sees (REQ-0637).
rebase_layer_paths <- function(spec, from_dir, entry_dir) {
  ...
  if (!is.null(spec$output)) {
    ...
    if (!is.null(spec$output$violation_log))
      spec$output$violation_log <- rebase(spec$output$violation_log)
  }
  spec
}
...
# keyed collections compose member by member, all other root fields
# replace their complete inherited value.
...
# intermediate or row; name identifies a column.
...
# A member id absent from the accumulated object is appended; a present
...
# fields merge key by key; the derivation composes as a registry value.
# Composition descends only while both values are of the same kind; a
...
# {value: ...}; the values compose as a registry value, other derivation
# fields replace.
...
# mappings; the same keyword composes a mapping payload key by key, while
# a different keyword -- or a non-mapping payload (sequences and scalars
...
# a canonicalized derivation. Mirrors deriv_refs' traversal but also
# collects qualifiers, so pruning can mark datasets and intermediates live.
...
# columns payload of a verification names output columns; predicates in
# implies/assert are parsed best-effort.
...
# output.columns, keys, order_by, and every column or dataset named by a
# verification. Columns not reachable from a root are dead; so are the
# inputs, intermediates and columns only they referenced (REQ-0639).
# Row-derivation entries targeting dead columns are removed with those
# columns. Verification-carrying columns are always live (a column's own
...
# execution-lifecycle dependency graph, topologically sorted; ties break
...
# last so entry-first fields keep their natural positions.
