...
# Accepts the numeric grammar's number with optional sign, plus the .inf/.nan
# spellings (which normalize to missing). Anything else fails.
...
# inside the signed 64-bit range. The clean-room int is an R integer
# (32-bit); values outside that range fail loudly rather than silently.
...
# Stage 2+3 of the derivation lifecycle: convert a derived value to the
# column's declared type, applying the `missing:` stage-3 handler per value.
# cf_present=FALSE (omitted) -> fatal; cf NULL (explicit) -> missing fallback.
