...
# spec directory (the R runner's root), else the benchmark's python/
...
# inline contract_version, or the shared `contract:` document keyed by
...
# project environment before any specification data is read. Returns the
# loaded environment (NULL when the spec calls no function).
...
# sourced project code (the local stand-in for the artifact's namespace),
# then the named R namespace for an explicitly declared stdlib binding
# such as stats::pnorm.
