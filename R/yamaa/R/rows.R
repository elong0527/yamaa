...
# apply the derivation's `missing:` stage-3 handler per value.
# REQ-0344/0359: omitted `missing:` makes conversion failure fatal; explicit
# null falls back to missing; a literal falls back to that literal.
...
# enclosing row template constructs. A window that depends on another
# window's result, directly or through a value computed from one, fails
# validation with window_on_window_result. This pass runs before the
# template's fixed-point evaluation. Dependency edges come from columns.R's
# deriv_refs (the shared derivation-reference extractor), scoped to the
# template's own derivation names.
...
# Stages 2-3 run here, during row construction, so dependents read converted
# values (REQ-0217).
