...
# A datetime or text source is the incompatible-input error; text is not
# parsed (a text source first becomes a date through to_date/date_impute).
...
# prefix fails invalid_date_text when no `invalid` handler is given
# (invalid_h NULL); a given handler (even NA) is used instead
...
# NULL (explicit null) -> NA; otherwise the handler value.
...
# source: ISO 8601 datetime text or complete date text (a datetime/date value
# is its canonical text, never invalid). A complete datetime is returned
# unchanged with collected precision "second"; a complete date receives the
# declared edge of its day (first -> T00:00:00, last -> T23:59:59) with
# collected precision "day". A source truncated before its day is invalid
# rather than receiving a second imputation policy.
# invalid_h "ABSENT" = key absent -> fail invalid_datetime_text;
# NULL (explicit null) -> NA; otherwise the handler value.
...
# "S" when the source carried a time, "D" when datetime_impute supplied it.
# Reading a datetime value binds the answer to the completed value; a
# datetime value is never invalid. Invalid text and a missing source behave
# as they do for datetime_impute (invalid_h NULL = absent -> fail).
