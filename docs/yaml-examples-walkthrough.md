...
| Question | Rule | Example |
|---|---|---|
| How do corporate, compound and study layers compose? | R017 | [`adam-adlb-standardized-result`](https://github.com/elong0527/yamaa/tree/main/yaml/examples/adam-adlb-standardized-result) -- three levels resolve `spec_organization -> spec_compound -> spec_study`, and `expected/spec_resolved.yaml` records the outcome: shorthand expanded to canonical form, member fields merged while root fields are replaced whole, unreachable declarations pruned, and layer-relative paths rebased to the entry file |
