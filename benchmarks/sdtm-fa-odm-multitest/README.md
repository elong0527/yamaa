...
long-form ODM diary, while carrying the reported reaction name and date from
the same diary occurrence.
...
tests. The same form repeat number is reused on another diary day.
...
test. A present severity item with a blank result produces an FA record
marked `NOT DONE`.
