...
**Input:** `input/odm.csv` is long-form ODM item data: one row per
collected item, identified by study (`StudyOID`), subject
(`SubjectKey`), event (`StudyEventOID` plus its repeat key),
item group, and item (`ItemOID`), with the collected answer in
`Value`. Each adverse event is one event occurrence: the event
repeat key distinguishes the two rashes, and the findings
collected for an event ride on the same occurrence. The
collected `IT.FA.AELNKID` item carries the link identifier
shared with the adverse event record, whose term is the
`IT.AE.AETERM` item on that same occurrence. No `dm.csv` is
included: the study and subject identifiers come straight from
the ODM rows, and no demographic values are needed.
...
- `FAOBJ` is the event term of the linked adverse event, read
  from the `IT.AE.AETERM` item on the same event occurrence
  (both events here are a rash).
- `FACAT` is always `AE`.
...
- `FASEQ` numbers the subject's records by linked event, then by
  test order (`LOC`, `SIZE`, `BIOPSY`); the collected result breaks
  any remaining ties so the numbering is fully determined.
- An adverse event with no findings form has no FA records.
- Every findings group must belong to a collected adverse event:
  `FAOBJ` names that event's term, and a group with no matching
  event fails the run instead of writing a record with no event
  term.
