...
context without reading across events. The reference dates come from
`dm.csv`, matched on the subject keys the two datasets share (`STUDYID`,
`USUBJID`).
...
- `SESTDTC` is the date the subject actually started the element: informed
  ...
  for follow-up (and for any other or missing epoch).
- `SEENDTC` is the date the subject actually ended the element: first dosing
  ...
  participation for follow-up (and for any other or missing epoch).
- `SESTDY` is the study day of the element start, counting the reference
  start date as day 1 with no day 0; blank when either date is missing.
- `SEENDY` is the study day of the element end, counted the same way.
- `SEUPDES` is always blank: the records are built only from planned
  elements, and the column describes only an unplanned one.
