...
non-negative `int`. `index` names the declared integer column that receives
...
demonstrates the missing-count failure contract. The
`adam-advs-once-measured-carry-forward` example expands the planned
subject/parameter spine before matching collected values, so an unattended
visit still has an analysis row.
