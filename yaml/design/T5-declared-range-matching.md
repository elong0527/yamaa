...
one current-row value and two bounds from the lookup dataset:
...
`value <= upper`. The value and bounds must be mutually comparable without
implicit conversion. A missing current-row value is an incomplete lookup; a
right-side record missing a bound is ineligible; and a complete value with no
eligible record is unmatched.
...
window table. Together they cover inclusive closed intervals, unmatched and
missing current-row values, equal-distance analysis-window ties, overlapping
intervals, and incomparable bound types.
