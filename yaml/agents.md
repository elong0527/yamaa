...
- Declare every source path as a relative file inside the example directory.
  ...
  so a rooted path names no approved location here, and it belongs with a
  URL, a parent traversal, and a symbolic link in a negative example. A
  rooted path is for a study whose runner approved a data root, not for a
  fixture this repository carries.
