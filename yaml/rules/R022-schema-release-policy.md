...
its release manifest. Bundle versions use Semantic Versioning 2.0.0, including
optional prerelease identifiers. All included schema documents have the exact
same version under R006.

The current `1.0.0-rc.1` bundle is a **prerelease**. The former `1.0` text was
an unissued design label, not a released bundle. It has no compatibility or
immutability promise. The first production release will be `1.0.0` only after
all indexed normative rules and executable conformance fixtures are accepted.
...
still increments the prerelease identifier or selects a new version. No change
may silently reuse a bundle version. Compatible additions therefore never
share an already published version.

## Publication and immutability

A production bundle is published only by an annotated Git tag named
...
in place. The `1.0-to-1.0.0-rc.1` fixture records the one-time conversion of
the unissued design label into the first version governed by this policy.
...
bundle version. It also checks the version format, uniform schema-module
versions, capability reports, migration coverage, and release manifest.
