...
type are deferred until an implementation environment is supplied. Omission
does
not create or maintain an environment implicitly.
**R018-2.** When actual project code is validated, activated, or executed, the
runner receives one explicitly selected project root. Before reading
specification data, it resolves exactly `environment.yaml` at that root. A
...
environments, parallel R and Python bindings, or a per-function language
choice.
...
contracts. Each name has exactly one contract and one binding in an
environment.
...
contract fingerprint. The payload is the following logical object, identified
by
...
**R018-11.** `params` is an array in declared order. Each entry contains
`name`,
...
| finite `float` | an object described below |
...
A finite `float` object has `type: "float"`. Its `value` is 16 lower-case
hexadecimal big-endian binary64 bits.

**R018-13.** `comparison_decimals` is its non-negative base-10 string. The
object is serialized as UTF-8 JSON under the JSON Canonicalization Scheme in
RFC
8785, then hashed with SHA-256 and prefixed with `sha256:`. Strings retain
their
R019 value. Parameters remain in their declared array order; object member
order
...
discovered pair of logical name and contract version to have the same
calculated
fingerprint. Two projects do not claim the same logical function contract
unless
these fingerprints are identical.


**R018-15.** Signatures are closed and named. Every parameter name is unique
and
...
**R018-17.** Every argument and default exactly matches its declared type.
There
...
Explicitly passing missing never selects the default. `accepts_missing`
defaults
...
mapping must cover the logical signature exactly. A Python host name is an
ASCII
...
the runner maps the logical arguments and invokes the callable once per logical
row. It supplies no undeclared data or execution context. The binding returns
one scalar of the declared exact type. Batch or vector execution is an
implementation optimization only when every observable value and failure equals
...
| `accepted-missing:name` | Named accepting parameter is explicitly missing |
| `short-circuit-missing:name` | Non-accepting parameter and result missing |
| `nullable-output` | An invoked nullable binding returns missing |
| `boolean-true:name`, `boolean-false:name` | Boolean parameter has value |
| `numeric-comparison` | Nonmissing `float` result tests decimals |
...
combination of environment version, artifact digest, every contract
fingerprint,
...
**R018-31.** Nonnumeric expected results and missingness compare by their
type's
...
four. At an exact decimal tie the copy is rounded to the nearest value away
from
...
runtime per project, pinned by digest and verified before activation, keeps
that
...
declared logical function: `unknown_project_function`. **R018-38.** A call
whose
