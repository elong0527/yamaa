...
value carries before conversion or normalization is reached. R014 applies
this rule's `str` row to a field's declared type, so text is parsed the same
way wherever it is read. R016 owns both temporal types. What a `date` and a
`datetime` denote, the text each type is read from and written back to, how
two of them order, and which operations read them are stated in R016. The
temporal cells below apply that rule rather than restating it. R018 owns the
...
them. The vocabularies share no value except coincidental spelling:
...
same runtime values. `date` and `datetime` are column types only.
`bool`, `"null"`, `list`, and `dict` are schema types only.
...
defines both. That rule states what each admits, the text each type is read
from and written back to, how two of them order, and which operations read
them. R011 adds nothing to that definition. A value neither type admits is
a `str` like any other. ISO 8601 text orders chronologically under R007
comparison.

**R011-7.** There is no Boolean column type. A flag is a `str` column with
an `allowed_values` verification, as the examples write a flag.
...
**R011-14.** After a project binding returns a host scalar, before R018
checks that binding's declared result contract.
...
missing values skip the value under R009, and an artifact carries the value
as the missing value its profile writes under R020. No artifact or canonical
value has an infinity or NaN spelling.
...
stored or quoted string stays text when the declared destination is `str`. Only
...
missing after normalization. The result is valid only when its R018
contract declares `may_return_missing: true`.

## Conversion

**R011-18.** At the point R005 defines, conversion applies the completed
derivation result to the declared column type. Conversion is deterministic.
...
**R011-20.** Conversion is not attempted for a missing input.
`conversion_failure` therefore does not fire, and a missing result is not
a failure.

**R011-21.** Parsing a `str` to a number accepts R010's `number` production
with an optional leading `+` or `-` and no surrounding whitespace. The parser
also recognizes exactly the YAML 1.2 core-schema non-finite forms: `.inf`,
...
back to, and the conversions the rule forbids are all stated in R016.
The collected precision a temporal value carries is stated in R016 too, and
canonical text carries the fields alone, so a `date` or `datetime` converted
to `str` does not carry the collected precision. R011 names R016 rather than
repeating the rule, so column conversion and every other reader agree.
...
A larger magnitude uses the nearest binary64 value.
...
written in positional notation. An integral value omits the trailing
...
The runtimes can agree only if one value has one text.
