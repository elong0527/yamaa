...
| `to_date` | `day` |
| selection expression | unchanged; the property travels |
...
separator, a lowercase `t`, a bare date read as a moment, and a trailing `Z`
are
...
| `2025-01-12T24:00` | `datetime` | hour 24; next day starts at `00:00` |
...
as the `--TM` family does, keeps the collected text as `str`. Admitting it
would
...
kinds of datetime in one column type, and the two target runtimes disagree
about
...
- **R016-21.** No civil time is nonexistent or ambiguous. A daylight-saving gap
  or repetition arises only in mapping a wall-clock reading onto a timeline,
  and
...
| Python | `datetime.date` | `datetime.datetime`; tzinfo unset; microsecond 0 |
| R | `Date` | `POSIXct` with `tzone` set to `"UTC"` |

**R016-26.** Integral seconds below 2^53 are exact in R's binary64
representation, which covers the whole of the year range above. R's `tzone` is
a
carrier and not a claim about the value: `"UTC"` is chosen because it is the
...
significant first: year, then month, day, and for a `datetime` hour, minute,
and
...
be between a civil time and an instant, so no comparison of two datetimes
fails.
...
denote something else -- the interval its collected components still admit, or
a
...
supplied, and can say so in the artifact it writes. A specification that needs
a
...
no implicit conversion between operation inputs, so a source list or an
ordering
...
**R016-39.** R011's conversion table routes every temporal cell here. What
those
cells apply is:

| Conversion | Result |
|---|---|
| `str` to `date` or `datetime` | parse the form above; otherwise fail |
| `date` to `str`, `datetime` to `str` | the canonical text above |
| `date` to `date`, `datetime` to `datetime` | identity |
| `date` to `datetime`, or reverse | fail; use a registered operation |
...
than as string surgery. Its result is a `date` like any other in every respect
a
...
**R016-44.** There is one precision ladder, ordered `year` before `month`
before
...
a temporal value it reports the collected precision that value carries, which
is
...
invent. Its default is `year`, preserving completion of both year-only and
year-
month sources. With `month`, a year-month source may receive the declared day,
while a valid year-only source produces missing because supplying both month
and
...
admit
an interval of days -- `2025` admits the whole of that year, `2025-01` the
whole
...
like falling below the minimum precision is neither a missing source nor
invalid
...
| `date_impute` | registered inputs | `date` |
| `date_precision` | `source` is `str` or `date` | `str` |
| `to_date` | `source` is `datetime` | `date` with collected precision `day` |

Registered `date_impute` inputs are `source` `str`, `month` `int`, and `day`
`int` or a month position. `minimum_source_precision` is `year` or `month`.
`not_before` is `date`.
...
`unit: day` between `2025-01-01T23:00:00` and `2025-01-02T01:00:00` could
defend
...
within the calendar ranges its registration states, and the date it completes
to
...
drops its time fields. A missing source returns a missing date. Any other
source
...
**R016-58.** This rule defines the text form once, so every place text becomes
a
...
before any derivation runs and no handler applies, while a `str` field
converted
...
the lexical form rejects everything but one extended shape per type;
portability
...
every pair of values of one type stays ordered; a specification that must act
...
including `date` to `datetime` and `datetime` to `date`: fail rather than
choose
a representation. **R016-64.** Comparing or ordering a temporal value against a
value of another type: fail under R007, which owns comparability. **R016-65.**
...
within the interval its collected components admit: missing, not a failure.
Like
a source below the minimum precision, it is neither a missing source nor
invalid
text, so no R008 handler answers it. **R016-70.** A temporal value used as an
identifiers. **R016-71.** Storing a value an implementation cannot hold
exactly,
such as a fractional or leap second: never reached, because the text is
rejected
