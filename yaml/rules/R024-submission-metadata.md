...
Every identifier this rule admits -- `standard`, `codelist`, and a document
reference -- names an object the study document declares. A specification
therefore carries the names without resolving them, and R026 resolves them and
applies every requirement below that depends on a standard's family.
...
**R024-3.** The free-form `metadata` map is unaffected. It remains
uninterpreted annotation that nothing validates, and no field below is read
from it. A study that carried governed facts in that map moves them here; the
two are not merged and the map is never a fallback.

## Standard families

**R024-4.** A dataset declares one `standard`. R026 resolves it to a declared
standard of type `IG`, and this rule reads that standard's published name as
one of three closed **families**:
...
**R024-5.** The table is closed. A dataset bound to an `IG` standard outside it
...
**R024-6.** A dataset must not bind to a standard of type `CT`. Controlled
terminology qualifies a codelist and not a dataset; R025 owns that binding.

## Dataset metadata

**R024-7.** `label` states what the dataset is, and is required. It is the
dataset's own description and is distinct from `domain`: `DM` is the domain
and `Demographics` is the label.

**R024-8.** `class` names the general observation class and is required, in
...
**R024-9.** `structure` states in prose the level of detail one record
...
**R024-10.** `repeating` is required and states whether the dataset may carry
...
**R024-11.** `domain` names the domain the dataset belongs to. It defaults to
...
**R024-12.** `domain` is declared only for a dataset whose family is `sdtm` or
...
**R024-13.** `data_type` states the type the submission represents the column
...
**R024-14.** A `str` column admits the temporal submission types because a
...
**R024-15.** `data_type` never changes a value. It states how a value R011
...
**R024-16.** `length` is the maximum expected value length, as the submission
...
**R024-17.** `length` is required when `data_type` resolves to `text`,
`integer`, or `float`, and must not be declared otherwise. The remaining
submission types carry values of a fixed written form, so a length for one
would be a second statement of that form.

**R024-18.** `significant_digits` is required when `data_type` resolves to
`float` and must not be declared otherwise. It is a non-negative integer
stating how many digits follow the decimal point.

**R024-19.** A `length` declared on a `str` column is enforced. The column's
completed values must contain at most `length` R019 scalar values, which is
exactly what R009's `max_length` requires, so the declared length is the
constraint rather than a note beside one. A specification declaring both
`length` and a `max_length` verification whose `max` differs is rejected; a
specification declaring both with the same bound is accepted, and the bound is
checked once.

**R024-20.** A `length` declared on any other column type is not enforced.
...
**R024-21.** `display_format` is presentation text carried for a reader. It
...
**R024-22.** `role` states how the column is used within its dataset, in the
...
**R024-23.** `role` must not be declared for a dataset whose family is `adam`.
That family defines no role vocabulary, so a role there would name nothing.

### Core and mandatory

**R024-24.** `core` is the standard's core designation: `Req` for required,
...
**R024-25.** `mandatory` states whether the completed column must carry a
value. It is not a second spelling of `core`, and the mapping between them is
standard-specific:

- **R024-26.** For `sdtm` and `send`, `core` is required and `mandatory`
  defaults from it: `Req` gives true, and `Exp` and `Perm` give false.
- **R024-27.** For `sdtm` and `send`, a declared `mandatory: true` on an `Exp`
  ...
  around it.
- **R024-28.** For `adam`, `mandatory` is required and nothing derives it.
...
**R024-29.** A column declaring `mandatory: true` must also carry a
...
**R024-30.** `codelist` names a codelist the study document declares. R025
owns that object, the values a binding enforces, and its agreement with an
`allowed_values` verification.

## Origin

**R024-31.** `origin` is required for every column carrying submission
...
**R024-32.** `origin.type` is one of `Assigned`, `Collected`, `Derived`,
...
**R024-33.** Which pairs are admitted depends on the family. For `sdtm`:
...
**R024-34.** For `adam`, the admitted types are `Derived`, `Assigned`,
`Predecessor`, and `Other`. `Derived`, `Assigned`, and `Other` require
`source: Sponsor`; `Predecessor` requires `source` to be absent. The standard
...
**R024-35.** For `send`, every type is admitted and `source` must be absent.
...
**R024-36.** `origin.description` is required when `type` is `Predecessor`,
...
**R024-37.** A column whose `origin.type` is `Collected` with a `source` of
`Investigator` or `Subject` must declare at least one document reference, and
R026 requires that document to be the study's annotated case report form. A
value said to be collected from a person is traceable to the form it was
collected on.

**R024-38.** A column whose `origin.type` is `Derived` must declare `method`.
...
**R024-39.** The derivation graph R001 builds proves some facts about a
...
**R024-40.** A column whose derivation is anything other than a bare `source`
...
**R024-41.** A column derived by a bare `literal` takes the same value in
...
**R024-42.** A column derived by a bare `source` copies one value of one
...
**R024-43.** A row-derived column is refuted by the derivation each `rows`
entry declares, applying R024-40 through R024-42 to every entry. Entries that
...
**R024-44.** `method` states the algorithm that produces a column's value.
...
**R024-45.** `description` is required, so a reader who cannot evaluate the
...
**R024-46.** `expression` carries code as text under a named `context`. This
...
**R024-47.** No `expression` is generated from a derivation. A derivation has
...
**R024-48.** `comment` carries a sponsor note about a dataset or a column.
...
**R024-49.** A document reference names a document the study document
...
**R024-50.** Every requirement above that does not depend on a family is
checked when the specification is validated on its own: the vocabularies, the
required and prohibited field combinations, the length binding in R024-19, the
graph refutations in R024-39 through R024-43, and the presence of `method` for
a declared `Derived` origin.

**R024-51.** Every family-dependent requirement is checked when R026 composes
...
**R024-52.** Submission metadata declared for a column outside
`output.columns`: fail validation and report the column.
**R024-53.** A `length` that is not a positive integer, or a
`significant_digits` that is negative: fail validation.
**R024-54.** A `length` absent where R024-17 requires it, or declared where it
prohibits it; a `significant_digits` absent or declared against R024-18: fail
validation with `submission_length_not_applicable` or
`submission_length_missing`, reporting the column.
**R024-55.** A `length` and a `max_length` verification declaring different
bounds on the same column: fail validation with `declared_length_conflict`,
reporting the column and both bounds.
**R024-56.** A `data_type` outside the set its declared column type admits:
fail validation with `submission_data_type_not_admitted`, reporting the column,
the declared type, and the submission type.
**R024-57.** A `core` declared for the `adam` family, absent for `sdtm` or
...
**R024-58.** A `mandatory: true` column with neither a `not_missing`
verification nor membership in `keys`: fail with `mandatory_not_enforced`.
**R024-59.** A `role` declared for the `adam` family, or a `domain` declared
for it: fail validation and report the field.
**R024-60.** An `origin` absent from a column carrying submission metadata:
fail with `origin_missing`.
**R024-61.** An origin type and source pair the family does not admit, a
`source` present where R024-33 through R024-35 require its absence, or a
`source` absent where they require it: fail with `origin_source_not_admitted`.
**R024-62.** An `origin.description` absent where R024-36 requires it: fail
with `origin_description_missing`.
**R024-63.** A `Collected` origin from `Investigator` or `Subject` with no
document reference: fail with `origin_document_missing`.
**R024-64.** A declared origin the derivation graph refutes under R024-40
through R024-43: fail with `origin_contradicts_derivation`, reporting the
column, the declared type, and the derivation that refutes it.
**R024-65.** A `Derived` origin with no `method`: fail with `method_missing`.
**R024-66.** A dataset bound to a standard outside R024-4's table, or to one
of type `CT`: fail with `unknown_standard_family`, reporting the dataset and
the standard.
**R024-67.** A `reference_data: true` dataset declaring `repeating: true`:
