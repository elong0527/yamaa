...
**R021-3.** A run may also receive **approved data roots**: existing local
...
it, never under a configuration of its own. When R018 also selects a project
...
**R021-5.** A runner bounds what a project configuration may approve. A runner
...
**R021-6.** The entry file resolves inside the approved project root. A layer
...
that R017 rebases to the entry file, and the rebased form must satisfy this
...
- **R021-11.** No empty segment and no trailing separator. Each is a
...
**R021-13.** A written path is ASCII under R019, like every other
repository-authored value.

## Resolution and file kind

**R021-14.** A relative `project_path` resolves relative to the directory of
...
**R021-15.** Every resolution has an **anchor**. A relative path is anchored
...
**R021-16.** Resolution walks the segments below the anchor one at a time from
...
**R021-25.** The written-form conditions are decided in the order R021-7
through R021-12 state them, then the anchor of R021-15, then the walk, so one
written path reports one condition on every platform.

**R021-26.** Every condition above is decided before any data is read and
reports under the `validation` phase, except `resource_path_content_changed`,
which reports under `ingest`.

- **R021-27.** An approved root that does not exist, is not a directory, or,
  for the approved project root, differs from an R018 project root: fail.
- **R021-28.** An entry file outside the approved project root: fail.
- **R021-29.** A project configuration that is not a mapping, carries an
