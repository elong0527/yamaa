...
reduction; this rule adds reduction and admits no window, `CASE`, comparison, or
Boolean construct.
...
**R013-1.** An `aggregate_expression` evaluates over the records of one relation
and returns one value per group. Its result is a single value for the group, so
the expression itself never changes row count: R003 joins a right-side reduction
...
**R013-2.** An identifier is `NAME` or `DATASET.NAME`, resolved as R002 resolves
the same name in the same phase, so a reducer expression and a predicate never
...
identifier only for a record already selected by an R015 record lookup; it still
...
**R013-7.** A qualified aggregate may declare `between` to narrow its right-side
...
missing stated bound is ineligible. In particular, a missing cutoff never causes
an implementation to reduce the unrestricted right side.
...
each later value is added to it using R010's `+` semantics; implementations must
...
median would additionally have to fix its interpolation rule before two runtimes
could agree, so it is not registered by default.

**R013-17.** `ONLY` counts records, not non-missing values. An eligible group
with one record returns that record's value even when it is missing. An eligible
...
**R013-18.** Reductions do not nest. The argument of a reduction must contain no
...
placeholder for the dataset named by the expression's qualified identifiers (for
example, `COUNT(EX.*)`). It is the one reducer that names no column, and it
counts records where `COUNT(x)` counts values.
...
`b` is grouped on, because a value that varies within the group gives the
expression no single answer, and silently taking one record's value would depend
...
- **R013-22.** An expression that is a single reduction retains that reduction's
  result type. `COUNT` returns `int`; `MEAN` returns `float`. `SUM` retains the
  numeric type of its argument. `MIN`, `MAX`, and `ONLY` retain the type they
  reduce, whatever that type is.
- **R013-23.** An expression using any operator or R010 function is numeric.
  Every reduction and every grouped identifier in it must be numeric, and R010's
...
**R013-25.** R011 converts the completed derivation result, as it does for every
...
**R013-28.** An uncollected quantity is therefore never reported as a measured
zero, and an absent record stays distinguishable from a collected missing value.
...
**R013-30.** `MEAN` answers missing before applying its defined division when no
...
fails on integer overflow for the same reason. Because `MEAN` is defined through
...
**R013-33.** A reduction does not sort the records it reads. `SUM` and therefore
...
defined division. Missing handling is pinned here because the candidate runtimes
...
inner reducers. **R013-38.** An identifier outside every reduction that is not a
...
`MAX` over values that are not mutually comparable: fail. **R013-47.** A window,
`CASE`, comparison, Boolean, string, subquery, or host-language construct: fail.
