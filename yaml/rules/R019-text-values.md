...
**R019-1.** Language source is ASCII. Schema documents, specifications,
...
**R019-2.** Data may contain Unicode. A runtime `str` is a finite sequence
of Unicode scalar values: code points `U+0000` through `U+D7FF` and `U+E000`
...
**R019-4.** No stage implicitly applies NFC, NFD, NFKC, NFKD, or any other
normalization. Ingestion, source binding, string conversion, concatenation,
interpolation, casing, mapping, grouping, comparison, key validation, and
artifact rendering preserve the scalar sequence except for the explicit
ASCII substitutions below.
...
**R019-6.** Two strings are equal exactly when they contain the same scalar
...
**R019-8.** Non-missing strings are ordered lexicographically by scalar
value. Compare the numeric code point at each position from left to right;
...
**R019-11.** `str_upper` replaces each scalar from `U+0061` through `U+007A`
with the scalar 32 positions earlier, from `U+0041` through `U+005A`.
Every other scalar is unchanged.

**R019-12.** `str_lower` replaces each scalar from `U+0041` through `U+005A`
with the scalar 32 positions later, from `U+0061` through `U+007A`. Every
other scalar is unchanged.

**R019-13.** Both operations preserve scalar count. They have no
...
**R019-14.** `mapping.case_sensitive: true` compares its source with
dictionary keys by the exact equality above.

**R019-15.** When `case_sensitive` is `false`, fold the source and every
...
**R019-16.** Identifier and keyword case behavior comes from the closed
...
R004. Schema patterns, `str_extract`, and `matches` match under R022 and
do not select a Unicode casing mode through this rule.

**R019-17.** Version 1.0 has no environment switch for non-ASCII casing.
...
**R019-18.** R and Python implementations must return the same scalar
sequences and order for the same inputs. Encoding equal results as UTF-8
produces byte-identical text. Fixtures must retain their exact bytes;
tooling must not normalize them while reading, writing, comparing, or
...
**R019-21.** Ill-formed encoded text or a decoded surrogate code point:
fail with `invalid_text` at the boundary where it enters.

**R019-22.** Dictionary keys that collide after the ASCII fold: fail
