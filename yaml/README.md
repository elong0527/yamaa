...
approved project root and whatever data roots its runner approved before
reading a specification, a declared path is a relative file inside the project
root or a rooted file inside an approved data root, with no URI scheme, no
escape above its root, and no symbolic link below it, and each accepted
...
keeps code and data in different places; it selects among approved roots and
never adds one.
