...
**REQ-1182.** A `datasets` entry's `dataset_json` names the Dataset-JSON file
...
**REQ-1183.** The study document declares it, and [Artifact publication](../storage/publication.md)'s extension
...
**REQ-1184.** A `dataset_json` path must differ from the document's
...
**REQ-1185.** An entry declaring `has_no_data` must not declare
...
**REQ-1186.** The generated file conforms to Dataset-JSON 1.1, and
...
**REQ-1187.** The newline-delimited form the standard also defines is not this
...
**REQ-1188.** The file's rows are the rows of the artifact the entry's
...
**REQ-1189.** Composition does not re-derive, re-order, filter, or round.
...
**REQ-1190.** Every metadata member of the file is generated from the same
...
**REQ-1191.** A specification a document composes already declares
...
**REQ-1192.** The file is one JSON object carrying these members, in this
...
| `metaDataRef` | the generated document, per [REQ-1195](dataset-json.md#req-1195) |
...
**REQ-1193.** A member whose source is absent is omitted. It is never written
...
**REQ-1194.** `rows` is written even when the dataset has no rows, as `[]`.
...
**REQ-1195.** `metaDataRef` is the document's `output.path` expressed relative
...
file names the document its `itemOID`s point into, and [REQ-1204](dataset-json.md#req-1204) has that
document name the file.

<a id="req-1196"></a>

**REQ-1196.** `sourceSystem` is one object carrying `name` and `version`, in
...
**REQ-1197.** `dbLastModifiedDateTime` and `targetDataType` are never written.
...
**REQ-1198.** Each `output.columns` entry produces one `columns` object
...
| `dataType` | per [REQ-1199](dataset-json.md#req-1199) |
...
**REQ-1199.** `dataType` maps [Submission metadata](metadata.md)'s resolved submission data type
...
**REQ-1200.** The table is not a second type system. Dataset-JSON's set is
...
**REQ-1201.** Each row is a JSON array holding one element per `columns`
...
**REQ-1202.** A temporal value is its canonical text and never a count from an
...
**REQ-1203.** A missing value is `null` and a collected empty string is `""`.
...
**REQ-1204.** When an entry declares `dataset_json`, the dataset's `def:leaf`
...
**REQ-1205.** The file is encoded UTF-8 and carries no byte-order mark.
`U+000A` terminates every line, including the last. `U+000D` is never written
outside a string escape.

<a id="req-1206"></a>

**REQ-1206.** The layout is fixed exactly:
...
**REQ-1207.** Within a string, `U+0022` is written `\"`, `U+005C` is written
...
**REQ-1208.** No number is written with an exponent, a leading `U+002B`, digit
...
**REQ-1209.** The bytes are fixed rather than the information. Two conforming
...
**REQ-1210.** A generation renders and validates every Dataset-JSON file and
...
**REQ-1211.** A failure at any point publishes nothing further and leaves
...
**REQ-1212.** This container is not a source. [Source ingestion](../storage/ingestion.md)'s extension mapping
stays closed at `.csv` and `.parquet`, and a `dataset_class.path` ending in
`.json` names no profile.

<a id="req-1213"></a>

**REQ-1213.** Two reasons, and each is enough. [REQ-1199](dataset-json.md#req-1199)'s mapping is not a
...
**REQ-1214.** The `dataset_json_path` interface has the following meanings. Shape, defaults, and
...
**REQ-1215.** A `dataset_json` path equal to the document's `output.path`, to
...
**REQ-1216.** A `dataset_json` declared on an entry that declares
`has_no_data`: fail validation with `dataset_json_without_data`, reporting the
dataset.

<a id="req-1217"></a>

**REQ-1217.** A declared `source_system` with no `source_system_version`, in a
document that declares any `dataset_json`: fail validation with
`source_system_incomplete`.

<a id="req-1218"></a>

**REQ-1218.** A `dataset_json` path, or a `metaDataRef` computed from it, that
...
**REQ-1219.** An artifact that has not been published, or whose fields, order,
...
**REQ-1220.** A generated file that is not valid against the Dataset-JSON 1.1
...
**REQ-1221.** A failed publication: fail and report the target, exactly as
