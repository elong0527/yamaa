...
| Field | Meaning |
| --- | --- |
| `dataset_class.path` | Source data path, relative to the declaring specification -- or to the project root when the run selected one -- or rooted at an approved data root and confined by [Resource resolution](resources.md); [CSV profile](csv.md) selects the source profile from its extension. |
