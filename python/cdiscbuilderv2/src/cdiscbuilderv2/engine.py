...
class CDISCEngine:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.datasets: Dict[str, pl.DataFrame] = datasets or input_data or {}
    ...
    def load_datasets(self) -> None:
        """Load datasets configured in 'input' or 'datasets' section if not already present in memory."""
        dataset_config = self.spec.get("input") or self.spec.get("datasets", {})
        if not isinstance(dataset_config, dict):
            return

        for alias, entry in dataset_config.items():
