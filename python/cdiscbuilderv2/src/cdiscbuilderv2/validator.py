...
class YamaaSchemaValidator:
    ...
    def _validate_root_fields(self) -> None:
        ...
        # Check input (primary post-#517) or datasets (legacy)
        input_block = self.spec.get("input")
        legacy_datasets = self.spec.get("datasets")

        if input_block is None and legacy_datasets is None:
            self.warnings.append("Root field 'input' is omitted (defaults to ODM input).")
        elif input_block is not None:
            if not isinstance(input_block, dict):
                self.errors.append("Field 'input' must be a mapping of {DatasetName: Path or Config}.")
        elif legacy_datasets is not None:
            if not isinstance(legacy_datasets, dict):
                self.errors.append("Field 'datasets' must be a mapping of {DatasetName: Path or Config}.")
            else:
                self.warnings.append("Root field 'datasets' is deprecated in Yamaa post-#517; use 'input' instead.")
    ...
    def _validate_filtered_source(self, path_ctx: str, source_obj: Any) -> None:
        """Validates a variable or filtered_source object under R002 / R003 / R008."""
        if isinstance(source_obj, str):
            if not source_obj.strip():
                self.errors.append(f"{path_ctx}: Source variable name cannot be empty.")
            return

        if not isinstance(source_obj, dict):
            self.errors.append(f"{path_ctx}: Source must be a variable string or mapping.")
            return

        var = source_obj.get("variable")
        if not var or not isinstance(var, str):
            self.errors.append(f"{path_ctx}: Source mapping is missing required 'variable' string.")
            return

        if "filter" in source_obj:
            filter_sql = source_obj["filter"]
            if not isinstance(filter_sql, str) or not filter_sql.strip():
                self.errors.append(f"{path_ctx}.filter: Filter must be a non-empty SQL string.")
            else:
                # R003-38: Prohibited construct - filter on unqualified source reading output column
                defined = getattr(self, "defined_cols", set())
                output_cols = [c.get("name") for c in self.spec.get("columns", []) if isinstance(c, dict)] if self.spec else []
                if "." not in var and (var in defined or var in output_cols):
                    self.errors.append(
                        f"Prohibited construct (R003-38) at {path_ctx}.filter: "
                        f"Source filter cannot be declared on unqualified output column '{var}', which has no records to select among."
                    )
                try:
                    SQLParser.parse_to_expr(filter_sql)
                except Exception as e:
                    self.errors.append(f"{path_ctx}.filter contains invalid SQL predicate '{filter_sql}': {e}")

        if "multiple_matches" in source_obj:
            mm = source_obj["multiple_matches"]
            if not isinstance(mm, dict):
                self.errors.append(f"{path_ctx}.multiple_matches must be a dictionary.")
            else:
                if "filter" in mm:
                    self.warnings.append(
                        f"{path_ctx}.multiple_matches: 'filter' inside multiple_matches is deprecated in post-#517; "
                        f"declare 'filter' beside multiple_matches on the source instead (R003-21a)."
                    )
                if "order_by" not in mm:
                    self.errors.append(f"{path_ctx}.multiple_matches is missing required 'order_by'.")
                if "keep" not in mm or mm["keep"] not in ("first", "last"):
                    self.errors.append(f"{path_ctx}.multiple_matches requires 'keep' to be 'first' or 'last'.")
    ...
    def _validate_derivation(self, col_name: str, deriv: Any) -> None:
        ...
        if "source" in deriv:
            recognized = True
            self._validate_filtered_source(f"columns.{col_name}.derivation.source", deriv["source"])
        ...
        if "mapping" in deriv:
            ...
            if not isinstance(m, dict):
                ...
                if "source" in m:
                    self._validate_filtered_source(f"columns.{col_name}.derivation.mapping.source", m["source"])
                else:
                    self.errors.append(f"Column '{col_name}' 'mapping' must contain 'source'.")
                if "dict" not in m:
                    self.errors.append(f"Column '{col_name}' 'mapping' must contain 'dict'.")

        if "coalesce" in deriv:
            recognized = True
            c = deriv["coalesce"]
            if not isinstance(c, dict) or "sources" not in c or not isinstance(c["sources"], list):
                self.errors.append(f"Column '{col_name}' 'coalesce' must contain a 'sources' list.")
            else:
                for idx, src in enumerate(c["sources"]):
                    self._validate_filtered_source(f"columns.{col_name}.derivation.coalesce.sources[{idx}]", src)
...
def auto_fix_yamaa_schema(raw_yaml: str, domain: Optional[str] = None) -> Tuple[str, List[str]]:
    ...
    if "datasets" in parsed and "input" not in parsed:
        parsed["input"] = parsed.pop("datasets")
        fixes.append("Migrated legacy root 'datasets' to 'input'")

    if "input" not in parsed:
        parsed["input"] = {"ODM": "input/odm.csv"}
        fixes.append("Added default 'input: {ODM: input/odm.csv}'")
