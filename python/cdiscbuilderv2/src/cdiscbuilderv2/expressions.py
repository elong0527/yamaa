...
class ExpressionParser:
    ...
    def _parse_expression(self, expr_def: Any, schema: Optional[dict] = None) -> pl.Expr:
        ...
        if "variable" in expr_def and not any(k in expr_def for k in ("source", "literal", "coalesce", "case", "mapping", "str_concat")):
            var = expr_def["variable"]
            base = self._parse_expression(var, schema)
            if "filter" in expr_def and expr_def["filter"]:
                try:
                    f_expr = SQLParser.parse_to_expr(expr_def["filter"], schema)
                    base = pl.when(f_expr).then(base).otherwise(None)
                except Exception:
                    pass
            if "missing" in expr_def and expr_def["missing"] is not None:
                base = base.fill_null(pl.lit(expr_def["missing"]))
            return base
        ...
        if key == "source":
            ...
            if isinstance(value, dict):
                ...
                if "filter" in value and value["filter"]:
                    try:
                        f_expr = SQLParser.parse_to_expr(value["filter"], schema)
                        base = pl.when(f_expr).then(base).otherwise(None)
                    except Exception:
                        pass
                if "missing" in value and value["missing"] is not None:
                    base = base.fill_null(pl.lit(value["missing"]))
                return base
            return pl.lit(None)
